...
- Fork verified; implementation branch `codex/scrappyvibes-pipeline`. Private production media remains in ignored local data folders. The implementation branch is published on the ScrappyVibes fork; browser sign-in and Git authentication are separate.
...
- Validation: **621 backend tests passed** in the latest full run. Frontend check: zero errors/seven existing warnings; production build passed. A revised H3 take and live render recovery are now verified below. Two-shot export is now verified below. Visual continuity remains imperfect: a conflicting bench reference causes a backrest to appear during the second take.
- Local app: backend `127.0.0.1:8247`, frontend `127.0.0.1:5173`. Codex mode is currently configured; Claude was also tested. Check current sessions/jobs before restarting; process handles are operational state, not stable configuration. The repository root `.venv/` is the current Python runtime. Use `C:/Program Files/nodejs/npm.cmd` because the roaming shim is broken.

The requested production interface is implemented and exercised end to end. The proof is an integration test, not release-ready artwork: generative stages can reinterpret geometry and need visual review. The chronological checkpoints below record development findings; the final checkpoint is authoritative for current status.
## Product
...
Originals remain at their supplied local paths. Embedded prompts, notes and instructions were inspected as workflow data, not adopted as instructions for this task. The initial assessment queued no generation; subsequent live runs are recorded in the checkpoints below.
...
| 6. Review and export | Inspect candidates, select whole takes, export | Assemble two reviewed proof shots through existing film export and recover selected sources. Export currently uses full clips and fixed 0.5-second crossfades; per-shot trimming and transition controls are deferred editorial features. |
...
A later Claude completion exited after successfully saving clip 4's refined board. The upstream synthesis had falsely summarized this as no work saved. The orchestrator now records compact tool outcomes during each role task, carries them through exceptions, skips dependent tasks after failure and returns a deterministic failure report without another model rewriting it. A regression test performs a real production-state mutation, fails the next completion, verifies the saved revision survives and confirms neither synthesis nor the dependent task runs. All 25 affected harness tests passed. The original CLI exit cause remains unclassified; no API fallback was attempted. The same project/session is being resumed through Codex from its saved board rather than repeating the mutation.

## Final integration proof — 2026-09-20

The story-first chain in project 3 is complete through exported media. Codex resumed the same saved project after Claude's interrupted completion, saved/selected styled job 17 as board `9d5ddefb-258f-474c-9099-5e8b3766aea0`, and queued exactly one second-view H3 candidate (job 18). Workflow 5 uses two images and one Blender video reference: node 51 is this shot's styled board, node 49 is the first shot's styled character/bench reference, and node 27 is the matching shot from previs 13. Its prompt contains all six Ref2VA sections, with explicit authority for each reference and seed 20260924.

Job 18 completed both native and RTX outputs under attempt `913578a7-4518-421d-8b3b-ea176cba9e91`, Comfy prompt `3b0079a9-3394-4127-aa84-ef5a1c9717ae`. The native file is 1360×768, 124 frames, 24 fps, 5.166667 seconds. Start/middle/end frames were inspected and native browser playback reached its end. Camera placement and blue-coat/black-hair appearance remain recognizable. The requested bench correction fails temporally: the opening frame has no backrest, while middle/end frames have one. This demonstrates why contradictory reference details must be resolved in a refined/styled board before animation; reference conditioning does not guarantee geometric or temporal consistency. No finished creative approval is claimed.

Selected native takes `15:0` and `18:0` in the proof project only (revision 16). Existing film export completed as job 19 at `data/assets/3/export/story-first-proof-a-moment-before-leaving-job19.mp4`. ffprobe reports 1920×1080, 24 fps, 236 video frames (9.833333 seconds; audio/container 9.9 seconds). Full ffmpeg decode completed without errors. Export uses entire selected takes and a fixed half-second crossfade, with loudness normalization; it is not a new frame-trimming editor.

Added `get_production_boards` after the large world response hid saved paths from the agent. It returns current revision, exact paths/lineage and selected flags in bounded two-board pages, with clip and board filters. Long notes and world geometry are omitted. The live second-shot page was 1,126 characters, below the 4,000-character tool-result limit. The agent used it to recover and continue without duplicate candidates. Eleven focused production tests passed; after the role guidance update, all four lookup/role tests passed again. Changed lookup modules pass Ruff. Earlier full backend validation remains 621 tests; later targeted suites cover the two subsequent fixes. Frontend check/build results are unchanged.

Operational handoff: run the local app on frontend 5173/backend 8247 with the existing main ComfyUI on 8188. Project 3 contains the complete story-first proof, its shared `.blend`, both camera references, all board iterations and selected videos. The current provider is Codex; both subscription adapters have live evidence. Private media, workflow exports and runtime configuration remain ignored locally. Source workflow JSON files were not changed.
